            ...
            .until_closes(async {
                'start_over: loop {
                    ...
                    if let Some(bl) = &mut old_boss_list {
                        ...
                        asr::print_message(&format!("Boss list updated: {:#?}", bl));
                    }
                    ...
                    'normal_game_loop: loop {
                        ...
                        match game_manager.read(&process, game_manager_inst) {
                            Ok(game_manager) if old_game_manager != Some(game_manager) => {
                                ...
                                match old_game_manager.map(|gm| gm.elevator_e_up) {
                                    Some(false)
                                        if game_manager.elevator_e_up && settings.first_elevator_up =>
                                    {
                                        asr::print_message("Split for first elevator up");
                                        timer::split();
                                    }
                                    _ => {}
                                }
                            ...
                            Err(err) => {
                                asr::print_message(&format!("game manager ERROR: {:?}", err));
                                continue 'start_over;
                            },
                            _ => {}
                        }
                        ...
                        match player_manager.read(
                            ...
                            Ok(player_manager) if Some(player_manager) != old_player_manager => {
                                match (player_manager, old_player_manager) {
                                    ...
                                    (player, _) => {
                                        timer::set_game_time(Duration::seconds_f32(player.time_played));
                                    }
                                }
                                ...
                                macro_rules! check_quest {
                                    ($field:ident, $msg:expr) => {
                                        match old_quest_manager.map(|am| am.$field) {
                                            Some(false) if quest_manager.$field && settings.$field => {
                                                asr::print_message(concat!("Split for ", $msg));
                                                timer::split();
                                            }
                                            _ => {}
                                        }
                                    ...
                                    ($field:ident, $alt_match:expr, $msg:expr) => {
                                        match old_quest_manager.map(|am| am.$field) {
                                            $alt_match if quest_manager.$field && settings.$field => {
                                                asr::print_message(concat!("Split for ", $msg));
                                                timer::split();
                                            }
                                            _ => {}
                                ...
                                macro_rules! check_ability {
                                    ($field:ident, $msg:expr) => {
                                        match old_ability_manager.map(|am| am.$field) {
                                            Some(false)
                                                if ability_manager.$field && settings.$field =>
                                            {
                                                asr::print_message(concat!("Split for ", $msg));
                                                timer::split();
                                            }
                        ...
                        if new_daruma_list != old_daruma_list {
                            ...
                            if let (Some(new_list), Some(old_list)) = (&new_daruma_list, old_daruma_list)
                            {
                                        ...
                                        && new_daruma.daruma_type == old_daruma.daruma_type
                                    {
                                        match new_daruma.daruma_type {
                                            DarumaType::Bite => {
                                                ...
                                                {
                                                    asr::print_message("Split for Chomper Daruma");
                                                    timer::split();
                                                }
                                            }
                                            DarumaType::Parry => {
                                                ...
                                                {
                                                    asr::print_message("Split for Mamori Daruma");
                                                    timer::split();
                                                }
                                            }
                                            DarumaType::Thorns => {
                                                ...
                                                {
                                                    asr::print_message("Split for Togi Chan Daruma");
                                                    timer::split();
                                                }
                                            }
                                            DarumaType::Spirits => {
                                                ...
                                                {
                                                    asr::print_message("Split for Jingu Daruma");
                                                    timer::split();
                                                }
                                            }
                                            DarumaType::Bomb => {
                                                ...
                                                {
                                                    asr::print_message("Split for Kaboomaru Daruma");
                                                    timer::split();
                                                }
                                            }
                                            ...
                                            DarumaType::FireWall => {
                                                ...
                                                {
                                                    asr::print_message("Split for PyroKun Daruma");
                                                    timer::split();
                                                }
                                            }
                                            DarumaType::Ice => {
                                                ...
                                                {
                                                    asr::print_message("Split for Yuki Daruma");
                                                    timer::split();
                                                }
                                            }
                                            DarumaType::Boomerang => {
                                                ...
                                                {
                                                    asr::print_message("Split for Ken Daruma");
                                                    timer::split();
                                                }
                                            }
                                        }
